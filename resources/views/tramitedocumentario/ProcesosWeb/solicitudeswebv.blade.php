                                                            ...
                                                            <tr style="font-size: 13px; padding-top: 3px; padding-bottom: 3px; height: 10px;">
                                                                ...
                                                                <td>
                                                                    ...
                                                                    @if($i->estado==1)
                                                                        ...
                                                                        {!! Form::open(['url'=>'tramitedocumentario/expediente/registrarv','id'=>"DSol-".$i->id,'method'=>'get','style'=>'display: inline;']) !!}
                                                                        <input type="hidden" name="id" value="{{$i->id}}">
                                                                        <button type="submit" onclick="return ConfirmarVerificar({{$i->id}})" class="btn btn-default btn-sm" title="Expediente"><span class="fa fa-plus"></span></button>
                                                                        {!! Form::close() !!}
                                                                    @else
                                                                        @include('tramitedocumentario.ProcesosWeb.movimientosSolicitudesWeb')
                                                                        <button type="button" class="btn btn-default btn-sm" data-toggle="modal" data-target="{{"#modal-movimientos-".$i->id}}" onclick="movimientos({{$i->id}});" title="Movimientos"><span class="fa fa-list-ol"></span></button>
                                                                        <button type="button" class="btn btn-default btn-sm" title="Elevado a expediente: {{$i->exped}}"><span class="fa fa-info"></span></button>
                                                                    @endif
                                                                </td>
                                                            </tr>
                                                        @endforeach
...
@section('scripts')
    <script>
        function ConfirmarVerificar(sol){
            ...
            if (confirm("¿Está seguro de elevar a expediente la solicitud web?")) {
                form.submit();
            }else{
                return false;
            }
